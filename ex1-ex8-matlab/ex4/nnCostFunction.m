...
for i = 1:m
    ...
    % compute delta terms for layers 2 and 3
    d3 = h - y_i; % 10x1
    
    
    d2 = (Theta2' * d3);
    d2 = d2(2:end, 1:end) .* sigmoidGradient(z2);    % 25x1 .* 25x1
...
Theta1_grad = Theta1_grad./m + [zeros(size(Theta1(:, 2))) reg_grad1];

reg_grad2 = lambda/m * Theta2(:, 2:end);
Theta2_grad = Theta2_grad./m + [zeros(size(Theta2(:, 2))) reg_grad2];


% regularization term
reg1 = Theta1(1:end, 2:end);
reg2 = Theta2(1:end, 2:end);
reg = (sum(sum(reg1.^2)) + sum(sum(reg2.^2)));
reg = reg*lambda/(2*m);
J = (J/m) + reg;
